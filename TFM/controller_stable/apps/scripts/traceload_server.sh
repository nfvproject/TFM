...
nc $KEEP -l $LISTEN_IP $LISTEN_PORT | while read COMMAND; do
    ...
    if [[ $ACTION == "start" ]]; then
    	if [[ $NUMPKTS == -1 && $RATE == -1 ]]; then
        	tcpreplay -i $INTERFACE --dbug=5 $TRACEDIR/$TRACE &
        elif [[ $NUMPKTS == -1 ]]; then
			tcpreplay -p $RATE -i $INTERFACE --dbug=5 $TRACEDIR/$TRACE &
        elif [[ $NUMPKTS == -1 ]]; then
			tcpreplay -L $NUMPKTS -i $INTERFACE --dbug=5 $TRACEDIR/$TRACE &
        else
			tcpreplay -L $NUMPKTS -p $RATE -i $INTERFACE --dbug=5 $TRACEDIR/$TRACE &
		fi
